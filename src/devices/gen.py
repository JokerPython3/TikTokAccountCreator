...
class DeviceGenerator:
    ...
    def __gen__device(self):
      ...
      if response.status_code != 200:
         return self.__gen__device()
      return {"response":response,"payload":payload,"headers":headers,"paramss":paramss}
    def verify(self) -> dict:
      try:
        data = self.__gen__device()
        res = data["response"]
        response = res
        payload = data["payload"]
        headers = data["headers"]
        paramss = data["paramss"]
      except Exception as e:
        return {"data":{"message":"device register request error","status":"ok","error":str(e)},"message":"error"}
      ...
      try:
        ...
        response = self.client.post(url, data=payload, headers=headers,cookies=self.cookies,params=params)
        print(response.text)
        
        if "device_id" in  response.text:
          with open(self.data + "devices.txt","a",encoding="utf-8") as f:
              f.write("{}:{}:{}:{}:{}:{}:{}:{}\n".format(device_id,iid,paramss["device_type"],paramss["device_brand"],paramss["cdid"],paramss["openudid"],self.cookies["passport_csrf_token"],headers["User-Agent"]))
          return {"data":{"message":"success","devices":{"device_id":device_id,"install_id":iid,"did":device_id,"iid":iid,"openudid":paramss["openudid"],"device_type":params["device_type"],"device_brand":paramss["device_brand"],"cdid":paramss["cdid"],"secrets":self.cookies["passport_csrf_token"],"user-agent":headers["User-Agent"]},"statuc":"ok"},"message":"success"}
        else:
           return {"data":{"message":"error coldnot verify devices","status":"ok"},"message":"error"}
      except Exception as e:
         return {"data":{"message":"device_id and install_id get error","status":"ok","error":str(e)},"message":"error"}
